...
| Version | Date | Change |
|:--|:--|:--|
| **3.82** | 2026-09-20 | **The PIN is six digits, and sign-in is one form instead of two steps — both at the owner's request, and the code changed first.** **The length was written in four places** — the domain policy, the request schema, the wizard's own validity check and the keypad's defaults — which is three chances for the till to accept what the server refuses, or to refuse what it would have taken. It is `@simon/shared`'s now (`pin.ts`), imported by both sides, for the reason §10.1 gives about the money rules: a rule both sides enforce is a rule that must exist once. **Four digits was 10,000 guesses.** §16.2's lockout makes that unguessable *online*, and the argument for six is not the lockout — it is §16.3: the same PIN re-authorises a discount above the cap and a blind return, typed at the counter in front of whoever asked for it. Six is the shortest that is not read over a shoulder and memorised on the spot, and it stays a number, because the people typing it are standing. **The two-step screen is merged.** It was never two requests — name and PIN always posted together, which is what makes an unknown name indistinguishable from a wrong PIN — so the split was a client-side artefact of the PIN pad, whose keyboard listener took every digit on the window and would have eaten a name typed beside it. The pad leaves this screen rather than being fixed: once there is a name field the device's keyboard is already up, a numeric field raises the numeric one, and the pad was spending a third of the screen duplicating it. §16.2's line holds unchanged, and holds more plainly — with no first step, there is no earlier answer to stop at. **A shop that upgrades cannot keep its PINs**: they are argon2 hashes, so a four-digit one cannot be lengthened, and every person needs a new one issued the way the first was — by the owner, in person (§16.4). §6's J1, §15.4, §16.2 and §11's `User.pinHash` follow |
...
1. Launch → the till (§6.1). Not signed in → the sign-in card: a name field and a PIN field, on
   one form.
2. Type your name and your PIN, then submit → `POST /auth/login` (§15.4). *Wrong PIN, or a name that
...
| `User.pinHash` | argon2id output. The PIN is exactly 6 digits and is never stored (§16.2) |
...
- **A person signs in by typing his name and his PIN, on one form** — `POST /auth/login
  {name, pin}` (§15.4), one request, which is what keeps the unknown-name case indistinguishable:
  there is no earlier step for the server to answer, so nothing can be learned by stopping after
  the name. The screen carries no keypad — a name field has already raised the device's keyboard,
  and a numeric field raises the numeric one. **Nobody is listed before sign-in**: no tiles, no faces, no route that answers without a
...
- Because the keyspace is tiny, the numbers are stated here rather than left to whoever writes
  the endpoint: the PIN is **exactly 6 digits**; **5 consecutive failures** lock the user for
...
| 2026-09-20 | **The PIN is exactly six digits**, and its shape lives once, in `@simon/shared` | §16.2, §11 `User.pinHash`, §16.3 | Not the online lockout, which already bounded guessing — the PIN is typed at the counter to approve a discount or a blind return (§16.3), and four digits is memorised over a shoulder. Existing PINs cannot be migrated: the owner issues new ones in person |
| 2026-09-20 | **Name and PIN are one form, not two steps** — the keypad leaves the sign-in screen | §16.2, §6 J1, §15.4 | The split was never a second request; it was the PIN pad's window-level key listener, which a name field beside it would have fed. Removing the pad costs nothing a raised keyboard does not already give, and §16.2's indistinguishable failure gets easier to hold, not harder |

...
export function catalogueRoutes() {
  ...
  // Quick-add from the till is open to every role (§7.4); the owner's extra fields are not.
  r.post("/products", async (req, res) => {
    const a = auth(req);
    const body = CreateProductBody.parse(req.body);
    const allowed = a.role === "ADMIN" ? body : { ...body, sku: null, categoryId: null, reorderPoint: undefined, reorderQty: undefined, trackStock: undefined };

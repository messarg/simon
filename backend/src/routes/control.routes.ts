 ...
 *
 * Every route here is ADMIN. Reports carry cost and margin, the audit trail carries whole-record
 * JSON, and diagnostics carry the shop's business — none of it is stripped field by field, because
 * §16.5 gates a payload like this as a route.
 */
...
export function controlRoutes() {
  ...
  r.get("/home", admin, async (req, res) => { res.json(await ownerHome(auth(req).db)); });

  r.get("/reports/:name", admin, async (req, res) => {
    const a = auth(req);
    ...
    res.json(await runReport(a.db, name as ReportName, { ...q, from: q.from ?? today, to: q.to ?? today }));
  });

...
import { isManager, isOwner, shapeMovement, stripCost } from "../lib/shape.ts";
...
export function buyRoutes() {
  ...
  const writeoff = requirePermission("writeoff");
  const admin = requireManager();
  const actor = (req: Express.Request) => { const a = auth(req); return { userId: a.userId, role: a.role, deviceId: a.deviceId }; };

  // Balances are for whoever runs the shop and terms for the owner alone; an employee who receives
  // sees who a supplier is, to receive against them (§16.5).
  r.get("/suppliers", receive, async (req, res) => {
    const a = auth(req);
    const { q } = z.object({ q: z.string().max(80).optional() }).parse(req.query);
    const rows = await listSuppliers(a.db, q);
    res.json({
      items: rows.filter((x) => isManager(a.role) || x.supplier.isActive === 1).map(({ supplier: s, books }) => (isManager(a.role)
        ? stripCost({ id: s.id, name: s.name, phone: s.phone, taxId: s.taxId, paymentTerms: s.paymentTerms, leadTimeDays: s.leadTimeDays, isActive: s.isActive === 1, outstanding: books.outstanding, overdue: books.overdue }, a.role)
        : { id: s.id, name: s.name, phone: s.phone, isActive: s.isActive === 1 })),
    });
  });
  r.post("/suppliers", receive, async (req, res) => {
    const a = auth(req);
    const s = await createSupplier(a.db, SupplierBody.parse(req.body), { details: isManager(a.role), terms: isOwner(a.role) });
  ...
  r.patch("/suppliers/:id", admin, async (req, res) => {
    ...
    // A manager cannot see a supplier's terms, so cannot be the one to set them.
    const s = await updateSupplier(a.db, a.userId, String(req.params.id), isOwner(a.role) ? body : { ...body, paymentTerms: undefined });
    res.json(stripCost({ id: s.id, name: s.name, phone: s.phone, taxId: s.taxId, paymentTerms: s.paymentTerms, leadTimeDays: s.leadTimeDays, isActive: s.isActive === 1 }, a.role));
  });
  ...
  r.get("/products/:id/receiving", receive, async (req, res) => {
    const a = auth(req);
  ...
  r.post("/goods-receipts", receive, async (req, res) => {
    const a = auth(req);
  ...
  r.get("/goods-receipts/:id", receive, async (req, res) => {
    const a = auth(req);
    const out = await loadReceipt(a.db, String(req.params.id));
    res.json(stripCost({ ...out.receipt, lines: out.lines, warnings: out.warnings }, a.role));
  });
  r.get("/goods-receipts", receive, async (req, res) => {
    const a = auth(req);
  ...
  r.post("/purchase-returns", receive, async (req, res) => {
    const a = auth(req);
  ...
  r.post("/write-offs", writeoff, async (req, res) => {
    const a = auth(req);
    res.json(shapeMovement(await writeOff(a.db, actor(req), WriteOffBody.parse(req.body)), a.role));
  });
  r.post("/adjustments", writeoff, async (req, res) => {
    const a = auth(req);
    res.json(shapeMovement(await adjustStock(a.db, actor(req), AdjustmentBody.parse(req.body)), a.role));
  });

  // Correcting a cost rewrites what a sale cost the shop: the owner's (§13.7, §16.4).
  r.post("/cost-corrections", requireOwner(), async (req, res) => {
    const a = auth(req);

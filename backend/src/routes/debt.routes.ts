...
import { shapeCustomer } from "../lib/shape.ts";
...
export function debtRoutes() {
  ...
  r.get("/customers", async (req, res) => {
    ...
    const rows = await listCustomers(a.db, { q: q.q, includeInactive: q.includeInactive === "true" && a.role === "ADMIN", limit: q.limit });
  ...
  r.get("/customers/snapshot", async (req, res) => {
    const a = auth(req);
  ...
  r.get("/customers/aging", async (req, res) => { res.json(await agingReport(auth(req).db)); });
  ...
  r.get("/customers/:id/ledger", async (req, res) => {
    const a = auth(req);
  ...
  r.post("/debt-payments", async (req, res) => {
    const a = auth(req);
    res.json(await submitDebtPayment(a.db, actor(req), DebtPaymentBody.parse(req.body)));
  });

  r.post("/debt-payments/:id/reverse", async (req, res) => {
    const a = auth(req);

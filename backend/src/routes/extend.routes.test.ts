...
const put = (t: TestApp, p: string, body: object, role: "WORKER" | "STOCK" | "ADMIN" = "STOCK") =>
...
describe("stocktake — §6.8, §13.4", () => {
  ...
  it("counts while the shop keeps selling, and a sale between snapshot and count is not shrinkage", async () => {
    ...
    const owner = (await get(t, `/stocktakes/${id}`, "ADMIN")).body;
    ...
    const approved = await post(t, `/stocktakes/${id}/approve`, {}, "ADMIN");
    ...
    await post(t, `/stocktakes/${id}/approve`, {}, "ADMIN");
    ...
    const history = (await get(t, "/stocktakes", "ADMIN")).body.items;
  ...
  it("an abandoned count posts nothing, and counting is not a worker's job", async () => {
    ...
    expect((await post(t, `/stocktakes/${id}/abandon`, {}, "ADMIN")).body.status).toBe("ABANDONED");
...
describe("purchase orders — §11, §13.2, §13.3", () => {
  ...
  beforeAll(async () => {
    ...
    await post(t, `/products/${cable.id}/units`, { uom: "կոճ", factorToStockUom: 50, role: "PURCHASE" }, "ADMIN");
    supplierId = (await post(t, "/suppliers", { id: uuidv7(), name: "Էլեկտրոտեխնիկա" }, "ADMIN")).body.id;
    await request(t.server).patch(`/api/suppliers/${supplierId}`).set(bearer(t.tokens.ADMIN)).send({ leadTimeDays: 5 });
  });
  afterAll(async () => { await t.close(); });
  ...
  it("is drafted by the owner, sent, received in two deliveries, and closes itself", async () => {
    ...
    const draft = await post(t, "/purchase-orders", body, "ADMIN");
    ...
    const revised = await request(t.server).put(`/api/purchase-orders/${body.id}`).set(bearer(t.tokens.ADMIN)).send(order([{ qty: 5_000, cost: 40_000_000 }]));
    expect(revised.body).toMatchObject({ total: 200_000 });
    expect((await post(t, `/purchase-orders/${body.id}/open`, {}, "ADMIN")).body.status).toBe("OPEN");
    expect((await request(t.server).put(`/api/purchase-orders/${body.id}`).set(bearer(t.tokens.ADMIN)).send(order([{ qty: 1_000, cost: 1 }]))).status).toBe(422);
    ...
    let po = (await get(t, `/purchase-orders/${body.id}`, "ADMIN")).body;
    ...
    expect((await post(t, `/purchase-orders/${body.id}/cancel`, {}, "ADMIN")).status).toBe(422);

    // The rest arrives counted in metres rather than spools: the order still recognises it.
    const metres = await post(t, `/products/${cable.id}/units`, { uom: "մետր", factorToStockUom: 1, role: "PURCHASE" }, "ADMIN");
    expect(metres.status).toBe(201);
    expect((await post(t, "/goods-receipts", receipt(body.id, 150_000, "մետր", 1), "STOCK")).status).toBe(200);
    po = (await get(t, `/purchase-orders/${body.id}`, "ADMIN")).body;
  ...
  it("refuses a delivery against another supplier's order, and receiving without an order still works", async () => {
    const other = (await post(t, "/suppliers", { id: uuidv7(), name: "Ուրիշ" }, "ADMIN")).body.id;
    const body = order([{ qty: 1_000, cost: 40_000_000 }]);
    await post(t, "/purchase-orders", body, "ADMIN");
    await post(t, `/purchase-orders/${body.id}/open`, {}, "ADMIN");
    ...
    expect((await post(t, `/purchase-orders/${body.id}/cancel`, {}, "ADMIN")).body.status).toBe("CANCELLED");
  });

  it("turns a reorder suggestion into a draft for the right supplier, in whole spools", async () => {
    await request(t.server).patch(`/api/products/${cable.id}`).set(bearer(t.tokens.ADMIN)).send({ reorderPoint: 1_000_000 });
    const suggestions = (await get(t, "/purchase-orders/suggestions", "ADMIN")).body.items;
    ...
    const made = await post(t, "/purchase-orders/from-suggestions", {}, "ADMIN");
    ...
    expect((await get(t, "/purchase-orders/suggestions", "ADMIN")).body.items).toHaveLength(1);
    await post(t, `/purchase-orders/${made.body.created[0].id}/open`, {}, "ADMIN");
    expect((await get(t, "/purchase-orders/suggestions", "ADMIN")).body.items).toEqual([]);
  });
});

...
describe("PIN login — §27.39", () => {
  ...
  const attempt = (userId: string, pin: string, deviceId: string) => request(t.server).post("/api/auth/login").send({ userId, pin, deviceId });
  ...
  it("an admin clears the lock in one action", async () => {
    ...
    expect((await request(t.server).post("/api/auth/unlock").set(bearer(t.tokens.ADMIN)).send({ userId: worker })).status).toBe(204);
    expect((await attempt(worker, PINS.WORKER, t.devices.WORKER)).status).toBe(200);
  });
  ...
  it("the owner's recovery code clears the only admin's lock, and is single-use", async () => {
    ...
    for (let i = 0; i < 5; i++) await request(server).post("/api/auth/login").send({ userId: owner.user.id, pin: "0000", deviceId: "d" + i });
    expect((await request(server).post("/api/auth/login").send({ userId: owner.user.id, pin: "4321" })).status).toBe(423);
    const rec = await request(server).post("/api/auth/recover").send({ userId: owner.user.id, recoveryCode: owner.recoveryCode });
    expect(rec.status).toBe(200);
    expect(rec.body.recoveryCode).not.toBe(owner.recoveryCode);
    expect((await request(server).post("/api/auth/recover").send({ userId: owner.user.id, recoveryCode: owner.recoveryCode })).status).toBe(401);
    expect((await request(server).post("/api/auth/login").send({ userId: owner.user.id, pin: "4321" })).status).toBe(200);
    server.close();
    await t2.close();
  });
...
describe("sessions and devices — §27.40 (device half), FR-SEC-06", () => {
  ...
  it("assigns each new device a unique two-character prefix", async () => {
    const { body } = await request(t.server).get("/api/devices").set(bearer(t.tokens.ADMIN));
  ...
  it("deactivating a device revokes its sessions in the same act", async () => {
    ...
    const res = await request(t.server).patch(`/api/devices/${t.devices.WORKER}`).set(bearer(t.tokens.ADMIN)).send({ isActive: false });
  ...
  it("an idle till session expires after fifteen minutes; the owner's does not", async () => {
    ...
    try {
      ...
      expect((await request(t.server).get("/api/auth/me").set(bearer(t.tokens.ADMIN))).status).toBe(200);
    } finally { clock.reset(); }
  });
  ...
  it("an admin revokes another session in one call, and it is audited", async () => {
    ...
    expect((await request(t.server).post(`/api/sessions/${me.session.id}/revoke`).set(bearer(t.tokens.ADMIN))).status).toBe(204);
...
describe("settings", () => {
  ...
  it("any session reads the client shape; only ADMIN reads or writes the whole", async () => {
    ...
    const patched = await request(t.server).patch("/api/settings").set(bearer(t.tokens.ADMIN)).send({ "discount.maxBp": 800 });
    expect(patched.body["discount.maxBp"]).toBe(800);
    expect((await request(t.server).patch("/api/settings").set(bearer(t.tokens.ADMIN)).send({ "discount.maxBp": 12.5 })).status).toBe(400);

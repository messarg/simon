...
export async function createTestApp(opts: { extraRouters?: express.Router[]; configure?: (t: TestDb) => Promise<void> } = {}): Promise<TestApp> {
  ...
  const loginAs = async (role: Role) => {
    const res = await request(app).post("/api/auth/login").send({ userId: users[role].id, pin: PINS[role], deviceLabel: `${role} till` });
  ...
  return { ...t, app, users, tokens, devices, loginAs };
}

...
- Access: three tiers, `OWNER` (exactly one; everything, and the only one who sees cost, backups, settings, sessions and the audit trail) · `MANAGER` (runs the shop, manages employees, never the owner's data) · `EMPLOYEE` (only granted permissions: `sell`, `returns`, `debt`, `receive`, `stocktake`, `writeoff`, `labels`). The rules are `packages/shared/src/{access,staff-policy}.ts`; routes gate with `requireOwner`/`requireManager`/`requirePermission` in `backend/src/middleware/auth.ts`. Sign-in lists nobody: a person types their name, then their PIN (`domain/person-name.ts`). The owner's session wears the dark `data-tier="owner"` palette in `theme.css`. ADR 0009.
...
- **Cost prices must be stripped server-side** for everyone but the owner — a manager included — and hiding them in the UI is not access control. Costs also travel inside a `ReviewFlag.note`, which no field-by-field rule would look inside: notes are stripped in `shapeFlag`.
...
- **Nobody is listed before sign-in, and an unknown name fails exactly as a wrong PIN** — same `pin-incorrect`, same time (a decoy argon2 verify). A route or a message that tells the two apart turns the sign-in screen back into the staff list (§16.2).

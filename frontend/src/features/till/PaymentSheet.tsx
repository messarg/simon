 ...
 */
import { Banknote, CheckCircle2, CreditCard, Printer, Split } from "lucide-react";
...
export interface PaymentSheetProps {
  ...
  total: number;
  onComplete: (payments: TenderInput[]) => Promise<CompletedSale>;
}
...
export function PaymentSheet({ open, onOpenChange, total, onComplete }: PaymentSheetProps) {
  const connection = useConnection();
  const [mode, setMode] = useState<"choose" | "cash" | "split">("choose");
  ...
  const finish = async (payments: TenderInput[]) => {
    if (busy) return;
    setBusy(true);
    try {
      setOfflineAtCompletion(connection === "offline");
      setDone(await onComplete(payments));
    } finally {
      setBusy(false);
    }
  };
            ...
            <div>
              ...
              <div className="mb-3 grid grid-cols-2 gap-3">
                ...
                <div className="rounded-lg bg-muted p-3">
                  <div className="text-sm text-muted-foreground">{t("payment.cardPart")}</div>
              ...
              <div className="mt-3 grid grid-cols-[auto_1fr] gap-2">
                <Button variant="secondary" size="xl" onClick={() => setMode("choose")}>{t("common.back")}</Button>
                <Button size="xl" disabled={busy || amount <= 0 || amount >= total} onClick={() => finish([{ method: "CASH", amount, tenderedAmount: amount }, { method: "CARD", amount: total - amount }])}>
                  {t("payment.complete")}
                </Button>
              </div>
            </div>

...
export function Toggle({ checked, onChange, label }: { checked: boolean; onChange: (v: boolean) => void; label: string }) {
  return (
    ...
    >
      ...
      <span className={cn("relative h-7 w-12 shrink-0 rounded-md transition-colors", checked ? "bg-primary" : "bg-border")}>
        <span className={cn("absolute top-0.5 left-0.5 size-6 rounded-xs bg-card shadow transition-transform", checked && "translate-x-5")} />
      </span>
    </button>
  );
}

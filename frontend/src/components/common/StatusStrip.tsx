  ...
  return (
    <div className={cn("safe-top border-b border-border bg-card/95 backdrop-blur", className)}>
      <div className="flex h-11 items-center gap-2 px-4 text-sm">
        <span className={cn("size-2 rounded-xs", shiftOpen ? "bg-success" : "bg-muted-foreground/40")} aria-hidden />
        ...
        {session?.session.mode === "PRACTICE" && (
          <span className="ml-1 rounded-xs bg-attention-soft px-2 py-0.5 text-xs font-medium text-attention-foreground">{t("status.practice")}</span>
        )}
        {counts.pendingSales > 0 && (
          <span className="ml-auto rounded-xs bg-attention-soft px-2 py-0.5 text-xs font-medium text-attention-foreground">{t("status.pending", { n: counts.pendingSales })}</span>
        )}
        {counts.needsAttention > 0 && (
          <Link to="/attention" aria-label={t("outbox.deviceTitle")} className="rounded-xs bg-destructive-soft px-2 py-0.5 text-xs font-medium text-destructive">! {counts.needsAttention}</Link>
        )}

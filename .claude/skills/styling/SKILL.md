---
name: styling
description: Tailwind CSS utility-first patterns for Simon — layout, flexbox, grid, spacing, typography, and colour, applied through semantic design tokens. Covers Tailwind v4 CSS-first configuration. Use when styling components or building layouts.
allowed-tools: Read, Write, Edit, Glob, Grep, Bash
---

> # ⚠️ PROJECT OVERRIDE — Simon
>
> This is a **generic, vendored** guide. Its examples use raw palette colours, `bg-opacity`, and
> `tailwind.config.js` — **none of which apply as-is here.** Where it conflicts with the rules
> below, **these rules win.** Translate every example before using it.
>
> 1. **Tailwind v4, CSS-first — no `tailwind.config.js`.** The theme lives in a CSS file via
>    `@theme inline` + CSS variables. Ignore all guidance about JS config, `content`/purge paths,
>    `darkMode: 'class'`, `jit`, or `safelist`.
> 2. **Colours: semantic tokens ONLY** — `bg-primary`, `text-foreground`, `text-muted-foreground`,
>    `bg-card`, `bg-muted`, `border-border`, `bg-destructive`. **Forbidden in components:**
>    Tailwind palette colours (`bg-white`, `text-gray-*`), arbitrary hex, raw colour values.
> 3. **Images:** plain `<img>` with explicit `width`/`height` to avoid layout shift — there is no
>    framework image component in a Vite SPA. Assets live under `public/assets/<subfolder>/` and
>    are referenced as `/assets/...`. **No external CDNs** — the shop may have no internet.
> 4. **Opacity: slash syntax** `bg-black/50` (v4) — never `bg-opacity-50` (v3).
> 5. **`components/ui/` is shadcn — DO NOT MODIFY.** Compose around it; reusable app components go
>    in `components/shared/` (barrel) or `components/common/`.
> 6. **Touch first.** This is a point-of-sale UI used one-handed, in poor light, under queue
>    pressure. Targets ≥ 48 px; totals large; no hover-only affordances. See the `ui-audit` skill.
> 7. **Do not add `dark:` variants speculatively.** A till screen is used in a lit shop; dark mode
>    is not a v1 requirement.
> 8. **Tooling: `npm`** (`npm run lint` = oxlint, `npm run build`, `npx tsc -b --noEmit`).
>
> Authoritative rules: the root `CLAUDE.md` + the project theme file. Audit with `ui-audit`.
...
> **Simon note:** the colour, config, dark-mode, and image examples below are generic v3-flavoured samples — read them through the **PROJECT OVERRIDE** banner above before applying anything.
...
5. **Configure Theme**: Customize design tokens in tailwind.config.js _(v4/Simon: no JS config — tokens live in `src/styles/theme.css` via `@theme inline`; see PROJECT OVERRIDE)_
...
### Opacity _(v4/Simon: use slash syntax `bg-primary/50`, not `bg-opacity-50`; see PROJECT OVERRIDE)_
...
## Dark Mode _(v4/Simon: no `darkMode: 'class'` config — declared via `@custom-variant dark` + a theme provider; dark tokens are unpopulated, so the app is light-mode only in practice. See PROJECT OVERRIDE)_
...
### Bundle Size Optimization _(v4/Simon: no `content`/purge config or `jit` — v4 scans automatically and this repo has no `tailwind.config.js`. See PROJECT OVERRIDE)_
...
### JavaScript Configuration (Legacy) _(v4/Simon: does NOT apply — this repo has no `tailwind.config.js`; theme extension happens in `src/styles/theme.css`. See PROJECT OVERRIDE)_
